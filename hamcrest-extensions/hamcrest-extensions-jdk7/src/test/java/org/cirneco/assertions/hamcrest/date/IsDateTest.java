...
public class IsDateTest extends BaseDateMatcherTest {
    ...
    public void testDescribeMismatchSafely() throws Exception {
        assertHasMismatchDescription(format("<%s> has not year <%d>", date, dateWrongYear),
                IsDate.hasYear(dateWrongYear), date);
        assertHasMismatchDescription(format("<%s> has not id <%d>", date, dateWrongMonth),
                IsDate.hasMonth(dateWrongMonth), date);
        assertHasMismatchDescription(format("<%s> has not day <%d>", date, dateWrongDay),
                IsDate.hasDay(dateWrongDay), date);

        assertHasMismatchDescription(format("<%s> has not year <%d>, id <%d>", date, dateWrongYear, dateWrongMonth),
                IsDate.hasYearAndMonth(dateWrongYear, dateWrongMonth), date);
        assertHasMismatchDescription(format("<%s> has not year <%d>, id <%d>, day <%d>", date, dateWrongYear, dateWrongMonth, dateWrongDay),
                IsDate.hasYearMonthAndDay(dateWrongYear, dateWrongMonth, dateWrongDay), date);

        //mismatch description covered by the constructor
        assertHasMismatchDescription(format("<%s> has not year <%d>, day <%d>", date, dateWrongYear, dateWrongDay),
                new IsDate(dateWrongYear, null, dateWrongDay), date);
        assertHasMismatchDescription(format("<%s> has not id <%d>, day <%d>", date, dateWrongMonth, dateWrongDay),
                new IsDate(null, dateWrongMonth, dateWrongDay), date);
    }

    @Test
    public void testDescribeTo() throws Exception {
        assertIsDescribedTo(format("a date with year <%d>", dateYear), IsDate.hasYear(dateYear));
        assertIsDescribedTo(format("a date with id <%d>", dateMonth), IsDate.hasMonth(dateMonth));
        assertIsDescribedTo(format("a date with day <%d>", dateDay), IsDate.hasDay(dateDay));

        assertIsDescribedTo(format("a date with year <%d>, id <%d>", dateYear, dateMonth), IsDate.hasYearAndMonth(dateYear, dateMonth));
        assertIsDescribedTo(format("a date with year <%d>, id <%d>, day <%d>", dateYear, dateMonth, dateDay),
                IsDate.hasYearMonthAndDay(dateYear, dateMonth, dateDay));

        //description covered by the constructor
        assertIsDescribedTo(format("a date with year <%d>, day <%d>", dateYear, dateDay),
                new IsDate(dateYear, null, dateDay));
        assertIsDescribedTo(format("a date with id <%d>, day <%d>", dateMonth, dateDay), new IsDate(null, dateMonth, dateDay));
    }
