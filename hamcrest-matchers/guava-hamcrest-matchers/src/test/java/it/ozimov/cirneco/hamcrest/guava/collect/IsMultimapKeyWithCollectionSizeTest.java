...
public class IsMultimapKeyWithCollectionSizeTest extends BaseMatcherTest {
    ...
    @Before public void mapUp() {
        ...
        isMultimapElementWithCollectionSizeMatcher =
            IsMultimapKeyWithCollectionSize.keyWithSize(comparison,
    ...
    @Test public void tesGivenAMultimapWithElementWithCollectionSizeTenWhenMatchesForSizeTenIsCalledThenTrueIsReturned()
        throws Exception {
    ...
    @Test public void tesGivenAMultimapWithElementWithCollectionSizeOneWhenMatchesForSizeTenIsCalledThenFalseIsReturned()
        throws Exception {

        //Arrange
        final int sizeCollectionForKeyInActualMap = 1;
        addObjectsToKey(sizeCollectionForKeyInActualMap);
        assumeThat(sizeCollectionForKeyInActualMap,
            not(is(expectedSizeCollectionForKeyInGeneralMatcher)));
    ...
    @Test public void testDescribeMismatchSafely() throws Exception {
        BaseMatcherTest.assertHasMismatchDescription(String.format(
                "Multimap had element \"%s\" with collection size <0>",
                comparison), isMultimapElementWithCollectionSizeMatcher,
            multimap);
    }

    @Test public void testDescribeTo() throws Exception {
        BaseMatcherTest.assertIsDescribedTo(String.format(
                "a Multimap with element \"%s\" with collection size <%d>",
                comparison, expectedSizeCollectionForKeyInGeneralMatcher),
            isMultimapElementWithCollectionSizeMatcher);
    }
