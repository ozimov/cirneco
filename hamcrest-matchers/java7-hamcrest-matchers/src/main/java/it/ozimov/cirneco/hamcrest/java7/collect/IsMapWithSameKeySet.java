 ...
 */
public class IsMapWithSameKeySet<K>
    extends TypeSafeMatcher<Map<? extends K, ?>> {
    ...
    @Override public boolean matchesSafely(final Map<? extends K, ?> map) {
        final Set<? extends K> mapKeySet = map.keySet();
        final Set<? extends K> comparisonMapKeySet = new TreeSet<>(
                comparisonMap.keySet());

        return mapKeySet.equals(comparisonMapKeySet);
    }

    @Override public void describeMismatchSafely(final Map<? extends K, ?> map,
        final Description mismatchDescription) {
        mismatchDescription.appendText("map key set was ").appendValueList("[",
            ", ", "]", map.entrySet()).appendText(
            " while expected key set was ").appendValueList("[", ", ", "]",
