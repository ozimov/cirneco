 ...
 */
public class IsBetweenUpperBoundInclusive<T extends Comparable<T>>
    ...
    @Override protected boolean matchesSafely(final T t) {
        return (t.compareTo(from) > 0) && (t.compareTo(to) <= 0);
    }

    @Override protected void describeMismatchSafely(final T item,
        final Description mismatchDescription) {
        mismatchDescription.appendValue(item).appendText(" is not between ")
            .appendValue(from).appendText(" excluded and ").appendValue(to)
            .appendText(" included");
    }

    @Override public void describeTo(final Description description) {
        description.appendText("a value between ").appendValue(from).appendText(
            " excluded and ").appendValue(to).appendText(" included");
    }
