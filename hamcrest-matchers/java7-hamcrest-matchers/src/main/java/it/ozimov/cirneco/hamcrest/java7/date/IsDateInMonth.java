...
public class IsDateInMonth extends TypeSafeMatcher<Date> {
     ...
     */
    private enum Month {
        JANUARY(Calendar.JANUARY), FEBRUARY(Calendar.FEBRUARY),
        MARCH(Calendar.MARCH), APRIL(Calendar.APRIL), MAY(Calendar.MAY),
        JUNE(Calendar.JUNE), JULY(Calendar.JULY), AUGUST(Calendar.AUGUST),
        SEPTEMBER(Calendar.SEPTEMBER), OCTOBER(Calendar.OCTOBER),
    ...
    public IsDateInMonth(final int month) {
        ...
        checkArgument(monthOptional.isPresent(),
            String.format(
                "The id %d is not a valid value (admitted values are [1,2,...,12])",
                month));
    ...
    @Override protected void describeMismatchSafely(final Date date,
        ...
        mismatchDescription.appendValue(date).appendText(" has not id ")
            .appendValue(month);
    }

    @Override public void describeTo(final Description description) {
        description.appendText("a date with id ").appendValue(month);
    }
