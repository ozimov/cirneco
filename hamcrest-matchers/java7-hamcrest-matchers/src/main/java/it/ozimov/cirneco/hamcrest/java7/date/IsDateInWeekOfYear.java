 ...
 * <p>
 * The matcher is compliant to the <em>ISO 8601</em> standard. Hence, a day at the end of December in year <em>X</em>
...
public class IsDateInWeekOfYear extends TypeSafeMatcher<Date> {
    ...
    public IsDateInWeekOfYear(final int weekOfYear) {
        checkArgument((weekOfYear >= 1) && (weekOfYear <= 53),
            "The value weekOfYear must be between 1 and 53");
    ...
    @Override protected void describeMismatchSafely(final Date date,
        ...
        mismatchDescription.appendValue(date).appendText(
            " is not in week of the year ").appendValue(weekOfYear);
    }

    @Override public void describeTo(final Description description) {
        description.appendText("a date in week of the year ").appendValue(
            weekOfYear);
    }
