...
public class IsPositiveInfinityTest extends BaseMatcherTest {
    ...
    @Test public void testGivenPositiveInfiniteNumberWhenMatchesIsCalledThenTrueIsReturned()
        ...
        //Act
        final boolean isFloatInfinity = isPositiveInfinityMatcher.matches(
                Float.POSITIVE_INFINITY);
        final boolean isDoubleInfinity = isPositiveInfinityMatcher.matches(
    ...
    @Test public void testGivenNonInfiniteNumberWhenMatchesIsCalledThenFalseIsReturned()
        ...
        //Act
        final boolean isByteInfinity = isPositiveInfinityMatcher.matches((byte)
                0);
        final boolean isShortInfinity = isPositiveInfinityMatcher.matches(
                (short) 0);
        ...
        final boolean isBigIntegerInfinity = isPositiveInfinityMatcher.matches(
                new BigInteger("0"));
        final boolean isAtomicIntegerInfinity =
            isPositiveInfinityMatcher.matches(new AtomicInteger(0));
        final boolean isAtomicLongInfinity = isPositiveInfinityMatcher.matches(
                new AtomicLong(0L));

        final boolean isFloatInfinity = isPositiveInfinityMatcher.matches(
                (float) .0);
        final boolean isDoubleInfinity = isPositiveInfinityMatcher.matches(.0D);
        final boolean isBigDecimalInfinity = isPositiveInfinityMatcher.matches(
                new BigDecimal(".0"));
    ...
    @Test public void testDescribeTo() throws Exception {
        assertIsDescribedTo("a value equals to positive infinity",
            isPositiveInfinityMatcher);
    }
