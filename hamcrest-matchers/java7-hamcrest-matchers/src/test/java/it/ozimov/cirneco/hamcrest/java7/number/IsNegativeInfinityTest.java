...
public class IsNegativeInfinityTest extends BaseMatcherTest {
    ...
    @Test public void testGivenPositiveInfiniteNumberWhenMatchesIsCalledThenTrueIsReturned()
        throws Exception {

        //Act
        final boolean isFloatInfinity = isNegativeInfinityMatcher.matches(
                Float.POSITIVE_INFINITY);
        final boolean isDoubleInfinity = isNegativeInfinityMatcher.matches(
    ...
    @Test public void testGivenNonInfiniteNumberWhenMatchesIsCalledThenFalseIsReturned()
        throws Exception {

        //Act
        final boolean isByteInfinity = isNegativeInfinityMatcher.matches((byte)
                0);
        final boolean isShortInfinity = isNegativeInfinityMatcher.matches(
                (short) 0);
        ...
        final boolean isBigIntegerInfinity = isNegativeInfinityMatcher.matches(
                new BigInteger("0"));
        final boolean isAtomicIntegerInfinity =
            isNegativeInfinityMatcher.matches(new AtomicInteger(0));
        final boolean isAtomicLongInfinity = isNegativeInfinityMatcher.matches(
                new AtomicLong(0L));

        final boolean isFloatInfinity = isNegativeInfinityMatcher.matches(
                (float) .0);
        final boolean isDoubleInfinity = isNegativeInfinityMatcher.matches(.0D);
        final boolean isBigDecimalInfinity = isNegativeInfinityMatcher.matches(
                new BigDecimal(".0"));
    ...
    @Test public void testDescribeMismatchSafely() throws Exception {
        assertHasMismatchDescription("<0> is not negative infinity",
            isNegativeInfinityMatcher, (byte) 0);
        assertHasMismatchDescription("<0s> is not negative infinity",
            isNegativeInfinityMatcher, (short) 0);
        assertHasMismatchDescription("<0> is not negative infinity",
            isNegativeInfinityMatcher, 0);
        assertHasMismatchDescription("<0L> is not negative infinity",
        ...
        assertHasMismatchDescription("<0.0F> is not negative infinity",
            isNegativeInfinityMatcher, .0F);
        assertHasMismatchDescription("<0.0> is not negative infinity",
            isNegativeInfinityMatcher, .0D);
        assertHasMismatchDescription("<InfinityF> is not negative infinity",
            isNegativeInfinityMatcher, Float.POSITIVE_INFINITY);
        assertHasMismatchDescription("<Infinity> is not negative infinity",
    ...
    @Test public void testDescribeTo() throws Exception {
        assertIsDescribedTo("a value equals to negative infinity",
            isNegativeInfinityMatcher);
    }
