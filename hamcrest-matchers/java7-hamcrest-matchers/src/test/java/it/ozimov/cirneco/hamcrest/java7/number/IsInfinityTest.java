...
public class IsInfinityTest extends BaseMatcherTest {
    ...
    @Test public void testGivenPositiveInfiniteNumberWhenMatchesIsCalledThenTrueIsReturned()
        ...
        //Act
        final boolean isFloatInfinity = isInfinityMatcher.matches(
                Float.POSITIVE_INFINITY);
        final boolean isDoubleInfinity = isInfinityMatcher.matches(
    ...
    @Test public void testGivenNonInfiniteNumberWhenMatchesIsCalledThenFalseIsReturned()
        ...
        final boolean isBigIntegerInfinity = isInfinityMatcher.matches(
                new BigInteger("0"));
        final boolean isAtomicIntegerInfinity = isInfinityMatcher.matches(
                new AtomicInteger(0));
        final boolean isAtomicLongInfinity = isInfinityMatcher.matches(
                new AtomicLong(0L));
        ...
        final boolean isBigDecimalInfinity = isInfinityMatcher.matches(
                new BigDecimal(".0"));
    ...
    @Test public void testDescribeMismatchSafely() throws Exception {
        assertHasMismatchDescription(
            "<0> is not infinite (positive or negative)", isInfinityMatcher,
            (byte) 0);
        assertHasMismatchDescription(
            "<0s> is not infinite (positive or negative)", isInfinityMatcher,
            (short) 0);
        assertHasMismatchDescription(
            "<0> is not infinite (positive or negative)", isInfinityMatcher, 0);
        assertHasMismatchDescription(
            "<0L> is not infinite (positive or negative)", isInfinityMatcher,
            0L);
        assertHasMismatchDescription(
            "<0> is not infinite (positive or negative)", isInfinityMatcher,
            new BigInteger("0"));
        ...
        assertHasMismatchDescription(
            "<0.0F> is not infinite (positive or negative)", isInfinityMatcher,
            .0F);
        assertHasMismatchDescription(
            "<0.0> is not infinite (positive or negative)", isInfinityMatcher,
            .0D);
        assertHasMismatchDescription(
            "<0.0> is not infinite (positive or negative)", isInfinityMatcher,
            new BigDecimal(".0"));
    ...
    @Test public void testDescribeTo() throws Exception {
        assertIsDescribedTo("a value equals to infinite (positive or negative)",
            isInfinityMatcher);
    }
