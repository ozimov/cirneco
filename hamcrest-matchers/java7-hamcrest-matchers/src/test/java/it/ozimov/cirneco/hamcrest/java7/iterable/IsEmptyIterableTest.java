...
public class IsEmptyIterableTest extends BaseIterableMatcherTest {
    ...
    @Test public void testGivenEmptyIterableWhenMatchesForSortedMatcherIsCalledThenTrueIsReturned()
        throws Exception {

        //Arrange
        final int expectedSize = 0;
        final Iterable<Object> emptyIterable = createIterableWithSize(
                expectedSize);
    ...
    @Test public void testGivenNonEmptyIterableWhenMatchesForSortedMatcherIsCalledThenFalseIsReturned()
        throws Exception {

        //Arrange
        final int expectedSize = 1;
        final Iterable<Object> nonEmptyIterable = createIterableWithSize(
                expectedSize);

        //Act
        final boolean matches = isEmptyIterableMatcher.matches(
                nonEmptyIterable);
    ...
    @Test public void testDescribeMismatchSafely() throws Exception {
        BaseMatcherTest.assertHasMismatchDescription("[<100>, <1>, <10>]",
            isEmptyIterableMatcher, createIterableWithElements(100, 1, 10));
    }

    @Test public void testDescribeTo() throws Exception {
        BaseMatcherTest.assertIsDescribedTo("an empty iterable",
            isEmptyIterableMatcher);
    }
